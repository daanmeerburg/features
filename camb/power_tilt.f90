    ...
    Type InitialPowerParams
        ...
        !pdm 2015 axion Monodromy as in Planck inflation paper 2015
#ifdef AXION1        

        !Semi-Analytical
        real(dl) log10f(nnmax)
        real(dl) deltans(nnmax)
        real(dl) deltaPhi(nnmax)
        real(dl) paxionf(nnmax)       
        real(dl) p_axion
        real(dl) phiEND
        real(dl) phiZERO
        real(dl) NEfoldStar
        !real(dl) paxionf(nnmax)
        !real(dl) log10alpha(nnmax)
        !real(dl) cone(nnmax)
        !real(dl) ctwo(nnmax)
    ...
    AP%paxionf = 0
    AP%p_axion = 4./3.
    AP%phiEND  = 0.59 !In units of M_p
    AP%phiZERO = 12.38 !In units of M_p
    AP%NEfoldStar = 57.5 
    !not used, but for pycamb     
    !AP%log10alpha = 0
    !AP%cone = 0
    !AP$ctwo = 0
#endif        
...
#ifdef AXION1
    NEfold0 = P%NEfoldStar+P%phiEND**2
    phik = sqrt(2*P%p_axion*(NEfold0+lnrat))
    ScalarPower=ScalarPower*(1.+  & 
         P%deltans(ix)*Cos(P%phiZERO*(phik/P%phiZERO)**(P%paxionf(ix)+1)/(10**(P%log10f(ix)))+P%deltaPhi(ix)))
    !write(*,*) k, ScalarPower
#endif
#ifdef AXION2
    ScalarPower=ScalarPower*(1.+ & 
         P%deltans(ix)*Cos(10**(P%log10alpha(ix))*(lnrat+P%cone(ix)*lnrat**(2)/P%NEfoldStar+ & 
         P%ctwo(ix)*lnrat**3/P%NEfoldStar**2) +P%deltaPhi(ix)))
#endif
...
#ifdef AXION1 
        InitPower%log10f(i) = Ini_Read_Double_Array_File(Ini,'log10f', i,0._dl)
        InitPower%deltans(i) = Ini_Read_Double_Array_File(Ini,'deltans', i,0._dl)
        InitPower%deltaPhi(i) = Ini_Read_Double_Array_File(Ini,'deltaphi',i,0._dl)
        InitPower%paxionf(i) = Ini_Read_Double_Array_File(Ini,'paxionf',i,0._dl)
#endif
#ifdef AXION2
        InitPower%log10alpha(i) = Ini_Read_Double_Array_File(Ini,'log10alpha', i,0._dl)
        InitPower%deltans(i) = Ini_Read_Double_Array_File(Ini,'deltans', i,0._dl)
        InitPower%deltaPhi(i) = Ini_Read_Double_Array_File(Ini,'deltaphi',i,0._dl)
        InitPower%cone(i) = Ini_Read_Double_Array_File(Ini,'cone',i,0._dl)   
        InitPower%ctwo(i) = Ini_Read_Double_Array_File(Ini,'ctwo',i,0._dl)
#endif
        if (WantTensors) then
