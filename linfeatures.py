...
initialparsLaTeX = ['n_s','\log_{10} \omega','C_{rm{lin}}','\delta_{\phi}','A_s']
initialpars = ['ns','log10_f','delta_ns','delta_Phi','As']
...
def resettemppars():
    ...
    tmp_initial_pars['delta_ns'] = 0.1 #this sets overall modulation of signal
...
#linear features very different Freq
Nfreq = 100
freq = np.linspace(-4.0,-2.5,Nfreq)
...
for n in range(0, Nfreq): 
    ...
    for i in range(0,totparams):
        sigma1D[i,n] = (InverseFisher[i,i])**(1./2.)
        sigmaM[i,n] = (MyFisher[i,i])**(-1./2.)
...
for i in range(0,totparams):
    if i < np.shape(cosmopars)[0]:
        ...
        storeas = cosmopars[i]+'_lin_error.pdf'
    else:
        yl,l1,l2, pltitle = createlatexlabel(initialparsLaTeX[i-int(np.shape(cosmopars)[0])])
        storeas = initialpars[i-int(np.shape(cosmopars)[0])]+'_lin_error.pdf' 
    plt.figure("fig1")
